				...
				if (jres) {
					...
					await this.$http({
						...
						if(res.data.loginCode===500083){
							...
							uni.navigateTo({
							 url:'../set/payPwd?phoneNumber='+res.data.phoneNumber
						   })
						}else{
							uni.showToast({
								title: '登录成功',
							...
							success: (info) => {
								...
								this.$http({
									...
									if (err.code === 500082) {
										...
										uni.redirectTo({
											url: '/pages/set/payPwd?phone='+err.data+'&flag=true'
										})
									}
								})
							}
...
<style lang='scss' scoped>
	...
	.container {
		...
		/*  #ifdef APP-PLUS || H5 */
		/* background-image: url('https://xmall-1300255297.cos.ap-guangzhou.myqcloud.com/cymall/img/loginbgk.png'); */
		/*  #endif */
		/*  #ifdef MP-WEIXIN */
